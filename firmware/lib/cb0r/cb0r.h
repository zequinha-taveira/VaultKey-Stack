...
// public domain UNLICENSE, contributions/improvements welcome via github at
// https://github.com/quartzjer/cb0r
...
// this struct is for representing parsed data only and retains pointers to the
// original bytes, it's not designed for composing types from scratch
typedef struct cb0r_s {
  ...
  cb0r_e type : 8;
  uint8_t header; // size of the type header bytes, start+head points to type's
                  // contents (if any)
} cb0r_s, *cb0r_t;

// low-level, pass raw CBOR bytes via start/stop pointers, returns end pointer
// (== stop if complete) can skip a given number of items and then will fill
// optional result w/ the current item
...
// find and fetch the value of a given key from the map, number/bytes args only
// used for some types
bool cb0r_find(cb0r_t map, cb0r_e type, uint64_t number, uint8_t *bytes,
               cb0r_t result);

// convenience method to write a header given a type and optional number
// (length/count/value), returns bytes written to out (max 9)
